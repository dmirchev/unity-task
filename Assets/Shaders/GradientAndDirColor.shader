Shader "Unlit/GradientAndDirColor"
{
	Properties {
	...
	SubShader {
      ...
      half _EmissiveStrengh;
		...
		void surf (Input IN, inout SurfaceOutput o) {
		 ...
		 finalColor += _ColorZ.rgb * max(0,dot(o.Normal, BACK));// * _ColorY.a;

         half3 gradient = lerp(_ColorLow, finalColor,  smoothstep( _yPosLow, _yPosHigh, IN.worldPos.y )).rgb;
         ...
         o.Emission = lerp(half3(0,0,0), gradient, _EmissiveStrengh);

         

         // the "color" before lighting is applied

         o.Albedo = gradient * saturate(1 - _EmissiveStrengh);
